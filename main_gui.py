...
class MonitorThread(QThread):
    ...
    # Сигналы для общения с главным потоком GUI
            ...
            try:
                ...
                if ssid_available:
                    ...
                    if is_connected:
                        ...
                        if has_internet:
                            status = f"✓ Подключено к {config.WIFI_SSID}, интернет доступен"
                            ...
                            status = f"⚠ Подключено к {config.WIFI_SSID}, но нет интернета"
                            
                        self.connection_changed.emit(True)
                        
                    else:
                        # Сеть доступна, но не подключены - пытаемся подключиться
                        status = f"Обнаружена сеть {config.WIFI_SSID}, подключаюсь..."
                    ...
                    status = f"✗ Сеть {config.WIFI_SSID} не обнаружена"
                    self.connection_changed.emit(False)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Информация о сети
        info_label = QLabel(f"Мониторинг сети: {config.WIFI_SSID}")
    ...
    def update_status(self, message, status_changed):
        ...
        # Обновляем нижний статус
        if "✓" in message:
            self.bottom_status.setStyleSheet("color: #27ae60; font-weight: bold; padding-top: 10px; border-top: 1px solid #ecf0f1;")
        elif "✗" in message or "⚠" in message:
            self.bottom_status.setStyleSheet("color: #e74c3c; font-weight: bold; padding-top: 10px; border-top: 1px solid #ecf0f1;")
        else:
            self.bottom_status.setStyleSheet("color: #7f8c8d; padding-top: 10px; border-top: 1px solid #ecf0f1;")
